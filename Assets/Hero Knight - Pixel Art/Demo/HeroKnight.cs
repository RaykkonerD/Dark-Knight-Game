...
public class HeroKnight : MonoBehaviour {
    ...
    void UpdateHearts()
    {
        ...
        {
            lifeHearts[i].enabled = i < baseLives;
        }
    }
    ...
    {
        if (other.gameObject.CompareTag("CoinSprite"))
        {
            int currentCoins = int.Parse(coinCounter.text);
            coinCounter.text = (currentCoins + 1).ToString();
            coinSound.Play();
            other.enabled = false;
        }
        else if (other.gameObject.CompareTag("LifePotionSprite"))
        {
            if (baseLives < lifeHearts.Count)
            {
                baseLives += 1;
                healSound.Play();
                UpdateHearts();
    ...
    void Update ()
    {
            ...
            {
                if (fallHeightRecorded)
                {
                    float fallDistance = m_fallHeight - transform.position.y;
            ...
            if (!m_groundSensor.State() && m_body2d.linearVelocity.y < 0)
            {
                if (!fallHeightRecorded)
                {
                    m_fallHeight = transform.position.y;
                    fallHeightRecorded = true;
                }
            }
            ...
            if (!m_rolling){
                ...
                {
                    platformVelocityX = currentPlatformRb.linearVelocity.x;
                }
